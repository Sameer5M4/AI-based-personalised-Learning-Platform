...
export default function CareerPath() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedOptions.skill !== '' && selectedOptions.level != null && selectedOptions.careerAspiration.length > 0 && selectedOptions.learningStyle.length > 0) {
      try {
        const response = await axios.post("http://127.0.0.1:5000/generate_quiz", {
          interested_skill: selectedOptions.skill,
          self_assessed_level: selectedOptions.level,
          goal: selectedOptions.careerAspiration,
        });
